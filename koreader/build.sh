...
# each into aidoku.koplugin/bin/<jit.arch>/aidoku-run, where <jit.arch> is
# LuaJIT's arch name (see main.lua) -- "arm" for armv7 Kindle/Kobo devices,
# "arm64" for aarch64 devices and desktop Linux.
set -euo pipefail
...
for target in "${targets[@]}"; do
    ...
    echo "building $jit_arch (GOARCH=$goarch${goarm:+ GOARM=$goarm})..."
